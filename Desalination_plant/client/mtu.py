...
from servers.plc1.plc1 import PLC1
from servers.plc2.plc2 import PLC2
from servers.plc3.plc3 import PLC3
from servers.plc4.plc4 import PLC4
from servers.plc5.plc5 import PLC5
...
class MTU:
    ...
    def logic_plc1(self, plc1_client: ModbusClient, logger: logging.Logger):
        ...
        if tank_lvl > PLC1.tank_thresholds['WARNING_HIGH']:
            if b1 != 0:
                plc1_client.write_single_register(PLC1.data_addresses['b1'], 0)
                logger.warning(f'Raw water tank almost full --> Input pump b1 turned off')
            if v1 != 1:
                plc1_client.write_single_register(PLC1.data_addresses['v1'], 1)
                logger.warning(f'Raw water tank almost full --> Output valve v1 opened')
        elif tank_lvl < PLC1.tank_thresholds['WARNING_LOW']:
            if b1 != 1:
                plc1_client.write_single_register(PLC1.data_addresses['b1'], 1)
                logger.warning(f'Raw water tank almost empty --> Input pump b1 turned on')
            if v1 != 0:
                plc1_client.write_single_register(PLC1.data_addresses['v1'], 0)
    ...
    def logic_plc2(self, plc1_client: ModbusClient, plc2_client: ModbusClient, logger: logging.Logger):
        ...
        if v1 != 0:
            if b2 != 1:
                plc2_client.write_single_register(PLC2.data_addresses['b2'], 1)
                logger.warning(f'Raw water output valve v1 open --> Water b2 pump turned on')
            if b3 != 1:
                plc2_client.write_single_register(PLC2.data_addresses['b3'], 1)
                logger.warning(f'Raw water output valve v1 open --> Coagulant b3 pump turned on')
        else:
            if b2 != 0:
                plc2_client.write_single_register(PLC2.data_addresses['b2'], 0)
                logger.warning(f'Raw water output valve v1 closed --> Water b2 pump turned off')
            if b3 != 0:
                plc2_client.write_single_register(PLC2.data_addresses['b3'], 0)
    ...
    def logic_plc3(self, plc2_client: ModbusClient, plc3_client: ModbusClient, logger: logging.Logger):
        ...
        if s2_flow > 0:
            if s2_salt > 6000:
                plc3_client.write_single_register(PLC3.data_addresses['b4'], 0)
                logger.warning(f'Detected inadequate filtered water inflow in s2 --> Pump b4 turned off')
            elif b4 != 1:
                plc3_client.write_single_register(PLC3.data_addresses['b4'], 1)
    ...
    def logic_plc4(self, plc3_client: ModbusClient, plc4_client: ModbusClient, logger: logging.Logger):
        ...
        if s4_flow > 0:
            if s4_flow_plc4 != s4_flow:
                plc4_client.write_single_register(PLC4.data_addresses['s4_flow'], s4_flow)
            if v2 != 1:
                plc4_client.write_single_register(PLC4.data_addresses['v2'], 1)
                logger.warning(f'Detected filtered water inflow in s4 --> NaClO valve v2 opened')
            if v3 != 1:
                plc4_client.write_single_register(PLC4.data_addresses['v3'], 1)
                logger.warning(f'Detected filtered water inflow in s4 --> CaO valve v3 opened')
            if v4 != 1:
                plc4_client.write_single_register(PLC4.data_addresses['v4'], 1)
                logger.warning(f'Detected filtered water inflow in s4 --> CO2 valve v4 opened')
        else:
            if s4_flow_plc4 > 0:
                plc4_client.write_single_register(PLC4.data_addresses['s4_flow'], 0)
            if v2 != 0:
                plc4_client.write_single_register(PLC4.data_addresses['v2'], 0)
                logger.warning(f'Not detected filtered water inflow in s4 --> NaClO valve v2 closed')
            if v3 != 0:
                plc4_client.write_single_register(PLC4.data_addresses['v3'], 0)
                logger.warning(f'Not detected filtered water inflow in s4 --> CaO valve v3 closed')
            if v4 != 0:
                plc4_client.write_single_register(PLC4.data_addresses['v4'], 0)
